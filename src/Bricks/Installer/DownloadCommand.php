...
/**
 * Abstract command used by commands which download and extract compressed Bricks files.
 *
...
abstract class DownloadCommand extends Command
{
     ...
     * @return $this
     *
     * @throws \RuntimeException If the Bricks archive could not be downloaded
     */
        ...
        try {
            ...
            if ('new' === $this->getName() && ($e->getCode() === 403 || $e->getCode() === 404)) {
                throw new \RuntimeException(sprintf(
                    ...
                    "Execute the following command to install the latest stable Bricks release:\n".
                    '%s new %s',
                    $this->version,
                    $_SERVER['PHP_SELF'],
                ...
                throw new \RuntimeException(sprintf(
                    "There was an error downloading %s from 20steps.de server:\n%s",
                    $this->getDownloadedApplicationType(),
                    $e->getMessage()
                ), null, $e);
            }
    ...
    /**
     * Extracts the compressed Bricks file (ZIP or TGZ) using the
    ...
    /**
     * Checks if environment meets Bricks requirements.
     *
     * @return $this
     */
    protected function checkBricksRequirements()
    {
        ...
        try {
            ...
            $bricksRequirements = new \SymfonyRequirements();
            $this->requirementsErrors = array();
            foreach ($bricksRequirements->getRequirements() as $req) {
    ...
    private function getBricksRequirementsFilePath()
    {
        $paths = array(
    ...
    /**
     * Creates the appropriate .gitignore file for a Bricks project if it doesn't exist.
     *
     * @return $this
     */
    ...
    /**
     * Returns the full Bricks version number of the project by getting
     * it from the composer.lock file.
     *
     * @return string The installed Bricks version
     */
    protected function getInstalledBricksVersion()
    {
        $bricksVersion = $this->composerManager->getPackageVersion('symfony/symfony');

        if (!empty($bricksVersion) && 'v' === substr($bricksVersion, 0, 1)) {
            return substr($bricksVersion, 1);
        };

        return $bricksVersion;
    }
     ...
     *
     * @param \Requirement $requirement The Bricks requirements
    ...
    /**
     * Generates a good random value for Bricks's 'secret' option.
     *
     ...
     *
     * @return bool Whether is Bricks 3
     */
    protected function isBricks3()
    {
    ...
    protected function checkInstallerVersion()
    {
        ...
        if (!$this->isInstallerUpdated()) {
            $this->output->writeln(sprintf(
                "\n <bg=red> WARNING </> Your Bricks Installer version (%s) is outdated.\n".
