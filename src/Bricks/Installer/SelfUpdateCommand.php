...
class SelfUpdateCommand extends AbstractDownloadCommand
{
    ...
    protected function configure()
    {
        ...
        $this
            ...
            ->setDescription('Update the Bricks Installer to the latest version.')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$forceUpdate && $this->isInstallerUpdated()) {
            $this->output->writeln(sprintf('// Bricks Installer is <info>already updated</info> to the latest version (%s).', $this->latestInstallerVersion));

            return;
        }

        $this->output->writeln(sprintf('// <info>updating</info> Bricks Installer to <info>%s</info> version', $this->latestInstallerVersion));
    ...
    /**
     * Downloads the new version of the Bricks Installer.
     *
     * @return $this
     */
    private function downloadNewVersion()
    {
        ...
        if (!is_writable($this->currentInstallerFile)) {
            throw new IOException('Bricks Installer update failed: the "'.$this->currentInstallerFile.'" file could not be written');
        }

        if (!is_writable($this->tempDir)) {
            throw new IOException('Bricks Installer update failed: the "'.$this->tempDir.'" directory used to download files temporarily could not be written');
        }

        if (false === $newInstaller = $this->getUrlContents($this->remoteInstallerFile)) {
            throw new \RuntimeException('The new version of the Bricks Installer couldn\'t be downloaded from the server.');
        }
    ...
    /**
     * Does a backup of the current version of the Bricks Installer.
     *
     * @return $this
     */
    ...
    /**
     * Replaces the currenct version of the Bricks Installer with the new one.
     *
     * @return $this
     */
    ...
    /**
     * Restores the previously installed version of the Bricks Installer.
     */
    private function rollback()
    {
        $this->output->writeln(array(
            ...
            'The previous Bricks Installer version has been restored.',
            '',
        ));
